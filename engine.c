...
#ifdef _TINSPIRE
	tmp = nSDL_LoadImage(image_logo);
#elif defined(DREAMCAST)
	tmp = SDL_LoadBMP("/cd/graphics/logo.bmp");
#else
	tmp = SDL_LoadBMP("graphics/logo.bmp");
#endif
...
void RotateLogo()
{
	...
	SDL_BlitSurface(logo, NULL, small_screen, &dst);
}
...
/*
...
#ifdef _TINSPIRE
	sprintf(name, "%i", (SDL_GetTicks() & 3) + 1);
	if (name == 4)
	{
		tmp = nSDL_LoadImage(image_title4);
		wh = 256;
		hh = 192;
	}
	else if (name == 2)
	{
		tmp = nSDL_LoadImage(image_title2);
		wh = 320;
		hh = 200;
	}
	else if (name == 3)
	{
		tmp = nSDL_LoadImage(image_title3);
		wh = 256;
		hh = 192;
	}
	else
	{
		tmp = nSDL_LoadImage(image_title1);
		wh = 320;
		hh = 200;
	}
#else
	...
	SDL_BlitSurface(title, &dst, small_screen, &dst);
...
void GameLoop()
{
	...
	static unsigned char world_loaded = 1;

	SetGameMode(GM_SPLASH);
#ifdef _TINSPIRE
	FILE *result;
	result = fopen("./lastmission.dat.tns", "r");
	if (result != NULL) fclose(result);
	game->world = load_world("./lastmission.dat.tns");
#elif defined(DREAMCAST)
	game->world = load_world("/cd/data/lastmission.dat");
#else
	game->world = load_world("data/lastmission.dat");
#endif

	/* If World file is in memory...
	 * then player can play the game properly
	 * */
	if (world_loaded == 1)
	{
		LoadLogo();
